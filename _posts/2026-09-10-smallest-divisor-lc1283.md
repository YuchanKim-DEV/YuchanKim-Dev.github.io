...
판정 함수(나눈 몫의 합을 구하는 부분)에서 지금은 배열을 끝까지 순회하며 합을 다 구하는데, 합이 threshold를 넘는 순간 바로 반복문을 끊어도(`if (sum > threshold) return false;`) 결과는 같다. 값이 커서 오버플로우가 걱정되는 경우나 배열이 아주 큰 경우엔 이런 조기 종료가 실질적인 성능 차이를 만든다. 로직 자체를 바꾸는 건 아니지만, "판정 함수는 답을 확정하는 순간 더 볼 필요가 없다"는 걸 의식해두면 좋다.
