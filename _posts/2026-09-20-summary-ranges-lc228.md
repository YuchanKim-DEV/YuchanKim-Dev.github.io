...
지금 짠 코드는 "이어지는 중인지"를 불리언(`count`)으로 추적하다가, 루프가 끝난 뒤 남은 마지막 구간을 별도 조건으로 처리하는 구조다. 대신 바깥 루프에서 "구간의 시작"을 잡고, 안쪽에서 그 구간이 끝나는 지점까지 아예 다 이동시켜버리는 이중 포인터 방식으로 짜면 "루프 종료 후 남은 구간" 같은 예외 처리 자체가 필요 없어진다.
...
바깥 `while`이 "다음 구간의 시작"으로 건너뛰고, 안쪽 `while`이 그 구간이 어디까지 이어지는지 끝까지 밀어버린다. 구간이 배열 끝에서 끝나든 중간에서 끝나든 같은 코드 경로를 타기 때문에, 오늘 겪었던 "마지막 구간만 따로 처리하다가 빠뜨리는" 유형의 버그 자체가 구조적으로 생길 수 없다.
