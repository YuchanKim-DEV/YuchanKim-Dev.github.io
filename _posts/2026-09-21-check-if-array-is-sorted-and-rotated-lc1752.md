...
`i`가 마지막 인덱스일 때 `(i + 1) % n`이 자동으로 `0`이 되기 때문에, `i == nums.length - 2`처럼 wraparound 지점을 따로 찾아서 예외 처리할 필요가 없다. 오늘 겪었던 시행착오(wraparound에서 `return false`를 잘못 넣었다가, 부등호 방향을 반대로 썼다가 하는 것)가 전부 "메인 루프와 다른 규칙을 wraparound에 새로 만들려는 시도"에서 나왔는데, `%` 연산자로 순환시키면 애초에 "다른 규칙"을 만들 여지 자체가 없어진다.
