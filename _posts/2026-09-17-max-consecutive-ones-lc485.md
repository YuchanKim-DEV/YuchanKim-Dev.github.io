...
지금 짠 "카운터 + 0에서 리셋" 패턴은 사실 **슬라이딩 윈도우**라는 더 큰 기법의 특수 사례다. 윈도우의 왼쪽 경계를 명시적인 변수로 관리하는 버전으로 바꾸면 이렇게 된다.
...
`count`를 늘렸다 줄였다 하는 대신, "0을 만나면 윈도우 왼쪽 경계를 그다음으로 밀어버린다"는 식으로 생각하는 것이다. 지금은 두 방식이 똑같아 보이지만, 나중에 "0이 최대 k개까지는 허용된다" 같은 변형 문제(연속 1의 최대 개수, k번까지 뒤집기 허용 등)를 만나면 카운터 방식은 한계에 부딪히고 이 윈도우 방식이 자연스럽게 확장된다.
