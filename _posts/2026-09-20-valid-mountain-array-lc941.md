...
지금 짠 방식은 `up`/`down`/`transition` 세 플래그로 상태를 추적하는데, 이 문제의 정석 풀이는 **양 끝에서 동시에 파고드는 투 포인터**다. 왼쪽에서는 오르막인 동안 전진하고, 오른쪽에서는(뒤에서부터 봤을 때) 오르막인 동안 후진한다. 두 포인터가 같은 지점에서 만나고, 그 지점이 양 끝이 아니면 산 모양이다.
